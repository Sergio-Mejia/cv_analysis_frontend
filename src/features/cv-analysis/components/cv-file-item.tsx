...
export function CvFileItem({ file, onRemove }: CvFileItemProps) {
  return (
    <li className="flex items-center gap-3 rounded-xl border border-panel-foreground/10 bg-panel-foreground/5 px-3 py-2.5">
      {file.previewUrl ? (
        // eslint-disable-next-line @next/next/no-img-element -- es un object URL (blob:) local, next/image no puede optimizarlo
        <img
          src={file.previewUrl}
          alt=""
          className="size-[42px] shrink-0 rounded-md object-cover"
        />
      ) : (
        <span
          aria-hidden="true"
          className="flex size-[42px] shrink-0 items-center justify-center rounded-md bg-panel-foreground/10 text-[10px] font-bold tracking-widest text-panel-foreground/75"
        >
          PDF
        </span>
      )}
